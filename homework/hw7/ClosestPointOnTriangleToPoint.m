function [closest_pt, line]= ClosestPointOnTriangleToPoint(vertices, pt)
...
%% projection to determine the normal point 
    ...
    elseif r> 1                            % projection exceeds b
        ...
        a1 = a;
        b1 = b;
        dist = [dist norm(pt-(vertices(:,i)+projection(a1,b1)))];
...
if normal(min_index) == 2                  % projection is on between vertices, normal point is the sum of projection and origin point
    closest_pt = vertices(:,min_index) + projection(veca(:,min_index),vecb(:,min_index));
    if min_index == 3                      % the normal point is on the line
        line = [vertices(:,3) vertices(:,1)];
    else
